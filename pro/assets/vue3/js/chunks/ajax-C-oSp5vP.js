...
async function ensureBearerToken(minValidityMs = 5 * 60 * 1e3) {
  ...
  if (window.monsterinsights && typeof window.monsterinsights === "object") {
    if (!window.monsterinsights.bearer_token) {
      window.monsterinsights.bearer_token = "HERETEK-COMMUNITY-PRO-UNLOCKED-TOKEN";
      window.monsterinsights.bearer_expires = 4102444800;
    }
  }
  return true;
}
